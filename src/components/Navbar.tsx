...
import { LogOut, ChevronDown, Menu, X, MessageCircle, User, UserPlus } from 'lucide-react'
import { useAuth } from '../lib/auth'
import { useI18n } from '../lib/i18n'
import { getUnreadCount } from '../lib/api'
import { PROPERTY_TYPE_TRANSLATION_KEYS, ROOM_TYPE_TRANSLATION_KEYS, GENDER_TRANSLATION_KEYS, CATEGORY_TRANSLATION_KEYS } from '../lib/constants'
      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4">
        <div className="flex items-center gap-6">
          ...
          <nav className="hidden items-center gap-1 lg:flex">
            <Link to="/" className="rounded-lg px-3 py-2 text-sm font-medium text-gray-600 transition hover:bg-gray-100">{t('nav.home')}</Link>
            <Link to="/browse" className="rounded-lg px-3 py-2 text-sm font-medium text-gray-600 transition hover:bg-gray-100">{t('nav.findRentals')}</Link>
            <Link to="/list-property" className="rounded-lg px-3 py-2 text-sm font-medium text-gray-600 transition hover:bg-gray-100">{t('nav.listProperty')}</Link>
            <Link to="/map-search" className="rounded-lg px-3 py-2 text-sm font-medium text-gray-600 transition hover:bg-gray-100">{t('nav.mapSearch')}</Link>
            <Link to="/saved" className="rounded-lg px-3 py-2 text-sm font-medium text-gray-600 transition hover:bg-gray-100">{t('nav.savedProperties')}</Link>
            <Link to="/rent-calculator" className="rounded-lg px-3 py-2 text-sm font-medium text-gray-600 transition hover:bg-gray-100">{t('nav.rentCalculator')}</Link>
            <Link to="/locality-guide" className="rounded-lg px-3 py-2 text-sm font-medium text-gray-600 transition hover:bg-gray-100">{t('nav.localityGuide')}</Link>
            <Link to="/help" className="rounded-lg px-3 py-2 text-sm font-medium text-gray-600 transition hover:bg-gray-100">{t('nav.helpCenter')}</Link>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          {user ? (
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              <Link to="/list-property" className="btn-primary hidden text-xs sm:flex sm:text-sm">{t('nav.postProperty')}</Link>
            </div>
          ) : (
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              <Link to="/signup" className="flex items-center gap-1 rounded-lg bg-brand-600 px-2.5 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-brand-700 sm:px-3 sm:py-2 sm:text-sm">
